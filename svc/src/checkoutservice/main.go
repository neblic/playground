...
type checkoutService struct {
	...
	emailKafkaAddr     string
	emailKafkaTopic    string
	emailKafkaProducer sarama.SyncProducer
...
func main() {
	...
	mustMapEnv(&svc.emailKafkaAddr, "EMAIL_KAFKA_ADDR")
	mustMapEnv(&svc.emailKafkaTopic, "EMAIL_KAFKA_TOPIC")
	...
	mustProduceKafka(ctx, &svc.emailKafkaProducer, svc.emailKafkaAddr)
	...
	if os.Getenv("ENABLE_NEBLIC") == "1" {
		unaryInterceptors = append(unaryInterceptors, neblicgrpc.UnaryServerInterceptor())
	}
	srv := grpc.NewServer(
...
func initNeblic() {
	...
	mustMapEnv(&controlServerAddr, "NEBLIC_CONTROL_SERVER_ADDR")
	mustMapEnv(&dataServerAddr, "NEBLIC_DATA_SERVER_ADDR")
	settings := sampler.Settings{
		ResourceName:      "checkoutservice",
		ControlServerAddr: controlServerAddr,
...
func mustConnGRPC(ctx context.Context, conn **grpc.ClientConn, addr string) {
	...
	if os.Getenv("ENABLE_NEBLIC") == "1" {
		unaryInterceptors = append(unaryInterceptors, neblicgrpc.UnaryClientInterceptor())
	}
...
func mustProduceKafka(ctx context.Context, producer *sarama.SyncProducer, addr string) {
	var err error
	ctx, cancel := context.WithTimeout(ctx, time.Second*3)
	defer cancel()
	...
	*producer, err = sarama.NewSyncProducer(strings.Split(addr, ","), config)
	if err != nil {
		panic(errors.Wrapf(err, "kafka: failed to connect %s", addr))
	}
}
...
func (cs *checkoutService) sendOrderConfirmation(ctx context.Context, email string, order *pb.OrderResult) error {
	sendOrderConfirmation := &pb.SendOrderConfirmationRequest{
		Email: email,
		Order: order,
	}
	marshaler := jsonpb.Marshaler{OrigName: true}
	value, err := marshaler.MarshalToString(sendOrderConfirmation)
	if err != nil {
		return err
	}

	_, _, err = cs.emailKafkaProducer.SendMessage(&sarama.ProducerMessage{
		Topic: cs.emailKafkaTopic,
